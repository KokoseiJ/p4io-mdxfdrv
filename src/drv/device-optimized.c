...
{
    /*
     * Rewritten to be more IO efficient,
     * It now makes a big read at once and iters through it to minimize
     * IO performance impacts.
     * Also moved to the bottom so the changes are easier to be seen.
     * Rest of the file should be identical to master branch of bemanitools.
     */
    ...
    do {
        read = aciodrv_port_read(device->fd, serial_buf, 1);
	...
	while (recv_size < expected_size) {
	    do {
		read = aciodrv_port_read(device->fd, serial_buf + serial_idx, 512 - serial_idx);
	    } while (read == 0);

	    if (read < 0) break;

	    serial_idx = 0;
	    while (serial_idx < read && recv_size < expected_size) {
		if (serial_buf[serial_idx] == AC_IO_ESCAPE) {
		    is_escape = 1;
		    serial_idx++;
		    continue;
		}
		if (!is_escape) {
		    recv_buf[recv_size++] = serial_buf[serial_idx++];
		    is_escape = 0;
		} else {
		    recv_buf[recv_size++] = ~serial_buf[serial_idx++];
		}
